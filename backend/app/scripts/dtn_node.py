...
async def dtn_flush_worker():
    ...
    async with httpx.AsyncClient() as client:
        while True:
            rows = fetch_queued_readings(limit=500)
            if not rows:
                await asyncio.sleep(SYNC_INTERVAL)
                continue
                
            logger.info(f"Attempting to transmit {len(rows)} buffered records to HQ via Protobuf...")
            ...
            try:
                ...
                if response.status_code == 201 or response.status_code == 200:
                    clear_synced_readings(row_ids)
                    logger.info(f"Successfully transmitted and cleared {len(rows)} records. Network Stable.")
                else:
